...
dataset_list = ['daily_dialog', 'empathetic_dialogues',
               'persona_chat', 'blended_skill_talk']
...
def merge_data(tokenizer, args):
   ...
   for data_name in dataset_list:
       ...
       if data_name == 'daily_dialog':
           ...
           part_train_dialogues, part_valid_dialogues, part_num_train, part_num_valid = load_empathetic(
               tokenizer, args.train_frac)
       elif data_name == 'persona_chat':
           part_train_dialogues, part_valid_dialogues, part_num_train, part_num_valid = load_persona(
               tokenizer, args.train_frac)
       elif data_name == 'blended_skill_talk':
           part_train_dialogues, part_valid_dialogues, part_num_train, part_num_valid = load_blended(
               tokenizer, args.train_frac)
...
if __name__ == '__main__':
   ...
   assert args.model_type in [
       ...
       "bolbolzaban/gpt2-persian", "sberbank-ai/mGPT"
   ]
   ...
   # train_dialogues, valid_dialogues, num_train, num_valid = merge_data(
   #     tokenizer, args)
 
   print("Saving train data...")
   save_data(args.train_prefix, args.data_dir, tokenizer)
   print("Saving validation data...")
   save_data(args.valid_prefix, args.data_dir, tokenizer)
